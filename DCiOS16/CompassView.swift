...
struct CompassView: View {
    ...
    var circleLabel: some View {
        CircleLabelView(diameter: 226).foregroundColor(.white)
    }
    ...
    var innerCircle: some View {
        Circle()
            .foregroundStyle(
                .linearGradient(colors: [Color(#colorLiteral(red: 0.03137254902, green: 0.0431372549, blue: 0.06666666667, alpha: 1)), Color(#colorLiteral(red: 0.3408924341, green: 0.3429200053, blue: 0.3997989893, alpha: 1))], startPoint: .topLeading, endPoint: .bottomTrailing)
    ...
    var innerCircle3: some View {
        Circle()
            .foregroundStyle(
                .radialGradient(colors: [Color(#colorLiteral(red: 0.1019607843, green: 0.1058823529, blue: 0.1529411765, alpha: 1)), Color(#colorLiteral(red: 0.262745098, green: 0.262745098, blue: 0.3215686275, alpha: 1))], center: .center, startRadius: 0, endRadius: 100)
                .shadow(.inner(color: .white.opacity(0.25), radius: 0, x: 1, y: 1))
                .shadow(.drop(color: .black.opacity(0.25), radius: 0, x: 1, y: 1))
            )
    }
    ...
    var background: some View {
        RadialGradient(colors: [Color(#colorLiteral(red: 0.283233285, green: 0.3070395291, blue: 0.4444938898, alpha: 1)), .black], center: .center, startRadius: 1, endRadius: 400)
            .ignoresSafeArea()
    }
    ...
    var strokes: some View {
        ZStack {
            ...
            Circle().strokeBorder(gradient, style: StrokeStyle(lineWidth: 10, dash: [1, 7]))
            Circle().strokeBorder(gradient, style: StrokeStyle(lineWidth: 15, dash: [1, 63]))
        }
        .frame(width: 315)
    }
